...
int gf_power_on_8996(struct gf_dev* gf_dev)
{
	...
	if (regulator_count_voltages(gf_dev->vreg) > 0) {
		//rc = regulator_set_voltage(gf_dev->vreg, 2800000,2800000);
		rc = regulator_enable(gf_dev->vreg);
		if (rc){
			printk(KERN_ERR "gf:Unable to set voltage on gfvdda");
		}
	}
#endif
	rc = regulator_enable(gf_dev->vreg);
	if (rc) {
		printk(KERN_ERR "gf:error enabling gfvdda %d\n",rc);
	}
	pr_warn("gf:%s, exit\n", __func__);
	return rc;
}
...
int gf_power_off_8996(struct gf_dev* gf_dev)
{
	...
	if (gf_dev->vreg) {
		...
		printk(KERN_ERR "gf:disable  gfvdda %d\n",rc);
	}
	pr_warn("gf:%s, exit\n", __func__);
	return rc;
}
