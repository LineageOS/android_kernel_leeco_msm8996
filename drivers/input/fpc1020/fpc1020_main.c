...
static int __init fpc1020_init(void)
{
	...
	{
            printk(KERN_INFO "spi_register_driver fail\n");
		return -EINVAL;
	}

        printk(KERN_INFO "spi_register_driver success\n");
	return 0;
}
...
static int __devinit fpc1020_probe(struct spi_device *spi)
{
	...
	if (!fpc1020)
	{
		dev_err(&spi->dev, "failed to allocate memory for struct fpc1020_data\n");
		return -ENOMEM;
	}
	...
	if (!fpc1020_pdata)
	{
		dev_err(&fpc1020->spi->dev, "spi->dev.platform_data is NULL.\n");
		error = -EINVAL;
		goto err;
	}
					...
					struct fpc1020_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->reset_gpio))
	{
		...
		dev_info(&fpc1020->spi->dev, "Assign HW reset -> GPIO%d\n", pdata->reset_gpio);
		...
		if (error)
		{
			dev_err(&fpc1020->spi->dev, "gpio_request (reset) failed.\n");
			return error;
		}
		...
		if (error)
		{
			dev_err(&fpc1020->spi->dev, "gpio_direction_output(reset) failed.\n");
			return error;
		}
	}
	else
					...
					struct fpc1020_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->irq_gpio))
	{
		...
		dev_info(&fpc1020->spi->dev, "Assign IRQ -> GPIO%d\n", pdata->irq_gpio);

		error = gpio_request(pdata->irq_gpio, "fpc1020_irq");

		if (error)
		{
			dev_err(&fpc1020->spi->dev, "gpio_request (irq) failed.\n");
		...
		if (error)
		{
			dev_err(&fpc1020->spi->dev, "gpio_direction_input (irq) failed.\n");
			return error;
		}
	}
	else
	...
	if (error)
	{
		dev_err(&fpc1020->spi->dev, "request_irq %i failed.\n", fpc1020->irq);
		fpc1020->irq = -EINVAL;
		return error;
	}
					...
					struct fpc1020_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->cs_gpio))
	{
		...
		dev_info(&fpc1020->spi->dev, "Assign SPI.CS -> GPIO%d\n", pdata->cs_gpio);

		error = gpio_request(pdata->cs_gpio, "fpc1020_cs");
		if (error)
		{
			dev_err(&fpc1020->spi->dev, "gpio_request (cs) failed.\n");
		...
		if (error)
		{
			dev_err(&fpc1020->spi->dev, "gpio_direction_output(cs) failed.\n");
			goto out_err;
		}
	}
	else
					...
					struct fpc1020_platform_data *pdata)
{
	...
	const void *boost_prop = of_get_property(node, "fpc,txout_boost_enable", NULL);
	const void *hwid_prop = of_get_property(node, "fpc,force_hwid", NULL);
	...
	if ((pdata->irq_gpio < 0) || (pdata->reset_gpio < 0) || (pdata->cs_gpio < 0))
	{
		dev_err(dev, "%s: Missing OF property gpio\n", __func__);
		goto of_err;
	}


	pdata->external_supply_mv = (vddtx_prop != NULL) ? be32_to_cpup(vddtx_prop) : 0;
	pdata->txout_boost = (boost_prop != NULL) ? 1 : 0;
	pdata->force_hwid = (hwid_prop != NULL) ? be32_to_cpup(hwid_prop) : 0;
