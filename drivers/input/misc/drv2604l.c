...
static struct drv2604l_data *drv2604l_data = NULL;

static unsigned int drv2604l_reg_read(struct drv2604l_data *haptics, unsigned int reg)
{
	unsigned int val;
	int ret;
...
static int drv2604l_reg_write(struct drv2604l_data *haptics, unsigned char reg, unsigned int val)
{
	return regmap_write(haptics->regmap, reg, val);
}

static int drv2604l_bulk_read(struct drv2604l_data *haptics, unsigned char reg, unsigned int count, u8 *buf)
{
	return regmap_bulk_read(haptics->regmap, reg, buf, count);
}

static int drv2604l_bulk_write(struct drv2604l_data *haptics, unsigned char reg, unsigned int count, const u8 *buf)
{
	return regmap_bulk_write(haptics->regmap, reg, buf, count);
}

static int drv2604l_set_bits(struct drv2604l_data *haptics, unsigned char reg, unsigned char mask, unsigned int val)
{
	return regmap_update_bits(haptics->regmap, reg, mask, val);
}

static int drv2604l_set_go_bit(struct drv2604l_data *haptics, unsigned int val)
{
	return drv2604l_reg_write(haptics, GO_REG, (val&0x01));
}

#if SKIP_AUTOCAL == 0
static void drv2604l_poll_go_bit(struct drv2604l_data *haptics)
{
	while (drv2604l_reg_read(haptics, GO_REG) == GO)
		schedule_timeout_interruptible(msecs_to_jiffies(GO_BIT_POLL_INTERVAL));
}
#endif

static int drv2604l_set_rtp_val(struct drv2604l_data *haptics, unsigned int value)
{
	/* Please, be noted: in unsigned mode, maximum is 0xff, in signed mode, maximum is 0x7f */
	return drv2604l_reg_write(haptics, REAL_TIME_PLAYBACK_REG, value);
}

static int drv2604l_set_waveform_sequence(struct drv2604l_data *haptics, unsigned char* seq, unsigned int size)
{
	return drv2604l_bulk_write(haptics, WAVEFORM_SEQUENCER_REG, (size>WAVEFORM_SEQUENCER_MAX)?WAVEFORM_SEQUENCER_MAX:size, seq);
}

static void drv2604l_change_mode(struct drv2604l_data *haptics, char work_mode, char dev_mode)
{
	...
	if (dev_mode == DEV_IDLE) {
		...
		if (haptics->dev_mode != DEV_STANDBY) {
			...
			drv2604l_reg_write(haptics, MODE_REG, MODE_STANDBY);
		...
		if ((work_mode != haptics->work_mode)
			...
			//	printk(KERN_DEBUG"%s:MODE_REAL_TIME_PLAYBACK\n", __FUNCTION__);
				drv2604l_reg_write(haptics, MODE_REG, MODE_REAL_TIME_PLAYBACK);
			} else if (haptics->work_mode == WORK_CALIBRATION) {
				drv2604l_reg_write(haptics, MODE_REG, AUTO_CALIBRATION);
			} else {
				drv2604l_reg_write(haptics, MODE_REG, MODE_INTERNAL_TRIGGER);
...
static void play_effect(struct drv2604l_data *haptics)
{
	switch_set_state(&haptics->sw_dev, SW_STATE_SEQUENCE_PLAYBACK);
	drv2604l_change_mode(haptics, WORK_SEQ_PLAYBACK, DEV_READY);
	drv2604l_set_waveform_sequence(haptics, haptics->sequence, WAVEFORM_SEQUENCER_MAX);
	haptics->is_playing = YES;
	drv2604l_set_go_bit(haptics, GO);

	while ((drv2604l_reg_read(haptics, GO_REG) == GO) && (haptics->should_stop == NO)) {
		schedule_timeout_interruptible(msecs_to_jiffies(GO_BIT_POLL_INTERVAL));
	}

	if (haptics->should_stop == YES) {
		drv2604l_set_go_bit(haptics, STOP);
	}

	drv2604l_change_mode(haptics, WORK_IDLE, DEV_STANDBY);
	switch_set_state(&haptics->sw_dev, SW_STATE_IDLE);
	haptics->is_playing = NO;
	wake_unlock(&haptics->wk_lock);
}

static void play_pattern_rtp(struct drv2604l_data *haptics)
{
	if (haptics->work_mode == WORK_PATTERN_RTP_ON) {
		drv2604l_change_mode(haptics, WORK_PATTERN_RTP_OFF, DEV_READY);
		...
		drv2604l_change_mode(haptics, WORK_PATTERN_RTP_ON, DEV_READY);
		hrtimer_start(&haptics->timer, ns_to_ktime((u64)haptics->vibration_time * NSEC_PER_MSEC), HRTIMER_MODE_REL);
	}
}

static void play_seq_rtp(struct drv2604l_data *haptics)
{
	if (haptics->rtp_seq.rtp_index < haptics->rtp_seq.rtp_counts) {
		...
		drv2604l_change_mode(haptics, WORK_SEQ_RTP_ON, DEV_READY);
		drv2604l_set_rtp_val(haptics,  rtp_val);
...
static void vibrator_off(struct drv2604l_data *haptics)
{
...
static void drv2604l_stop(struct drv2604l_data *haptics)
{
	if (haptics->is_playing) {
		if ((haptics->work_mode == WORK_VIBRATOR)
				||(haptics->work_mode == WORK_PATTERN_RTP_ON)
...
{
	struct drv2604l_data *haptics = container_of(dev, struct drv2604l_data, to_dev);
...
static void vibrator_enable( struct timed_output_dev *dev, int value)
{
	...
	drv2604l_stop(haptics);
	//printk(KERN_DEBUG"%s:value:%d\n", __FUNCTION__, value);
	if (value > 0) {
		wake_lock(&haptics->wk_lock);

		drv2604l_change_mode(haptics, WORK_VIBRATOR, DEV_READY);
		haptics->is_playing = YES;
...
{
	struct drv2604l_data *haptics = container_of(timer, struct drv2604l_data, timer);
...
{
	struct drv2604l_data *haptics = container_of(work, struct drv2604l_data, vibrator_work);
...
{
	struct drv2604l_data *haptics = context;
	...
	if (fw != NULL) {
		...
		memcpy(&(haptics->fw_header), pbuf, sizeof(struct drv2604l_fw_header));
		if ((haptics->fw_header.fw_magic != DRV2604L_MAGIC)
			...
			drv2604l_change_mode(haptics, WORK_IDLE, DEV_READY);

			pbuf += sizeof(struct drv2604l_fw_header);

			drv2604l_reg_write(haptics, RAM_ADDR_UPPER_BYTE_REG, 0);
			drv2604l_reg_write(haptics, RAM_ADDR_LOWER_BYTE_REG, 0);

			fwsize = size - sizeof(struct drv2604l_fw_header);
			for (i = 0; i < fwsize; i++) {
				drv2604l_reg_write(haptics, RAM_DATA_REG, pbuf[i]);
			}
...
static int dev2604l_open (struct inode * i_node, struct file * filp)
{
	if(drv2604l_data == NULL) {
		return -ENODEV;
	}

	filp->private_data = drv2604l_data;
	return 0;
}

static ssize_t dev2604l_read(struct file* filp, char* buff, size_t length, loff_t* offset)
{
	struct drv2604l_data *haptics = (struct drv2604l_data *)filp->private_data;
	int ret = 0;
...
static ssize_t dev2604l_write(struct file* filp, const char* buff, size_t len, loff_t* off)
{
	struct drv2604l_data *haptics = (struct drv2604l_data *)filp->private_data;
	...
	case HAPTIC_CMDID_PLAY_EFFECT_SEQUENCE: {
		...
		if (!copy_from_user(&haptics->sequence, &buff[1], len - 1))
		{
			...
			drv2604l_change_mode(haptics, WORK_SEQ_PLAYBACK, DEV_IDLE);
			schedule_work(&haptics->vibrator_work);
		}
		break;
	}
	case HAPTIC_CMDID_PLAY_TIMED_EFFECT: {
		...
		if (value > 0) {
			...
			haptics->is_playing = YES;
			value = (value > MAX_TIMEOUT)?MAX_TIMEOUT:value;
			drv2604l_change_mode(haptics, WORK_RTP, DEV_READY);
	...
	case HAPTIC_CMDID_PATTERN_RTP: {
		...
		if (haptics->vibration_time > 0) {
			...
			drv2604l_change_mode(haptics, WORK_PATTERN_RTP_ON, DEV_READY);
			drv2604l_set_rtp_val(haptics, strength);
	...
	case HAPTIC_CMDID_RTP_SEQUENCE: {
		...
		if (((len-1)%2) == 0) {
			...
			if ((haptics->rtp_seq.rtp_counts <= MAX_RTP_SEQ)
				...
				wake_lock(&haptics->wk_lock);
				switch_set_state(&haptics->sw_dev, SW_STATE_RTP_PLAYBACK);
				drv2604l_change_mode(haptics, WORK_SEQ_RTP_OFF, DEV_IDLE);
				schedule_work(&haptics->vibrator_work);
			} else {
	...
	case HAPTIC_CMDID_UPDATE_FIRMWARE: {
		...
		if (fw_buffer != NULL) {
			...
			if (result == 0) {
				...
				drv2604l_firmware_load(&fw, (void *)haptics);
			}
			wake_unlock(&haptics->wk_lock);
	...
	case HAPTIC_CMDID_READ_FIRMWARE: {
		...
		if (len == 3) {
			haptics->r_len = 1;
			drv2604l_reg_write(haptics, RAM_ADDR_UPPER_BYTE_REG, buff[2]);
			drv2604l_reg_write(haptics, RAM_ADDR_LOWER_BYTE_REG, buff[1]);
			haptics->r_buff[0] = drv2604l_reg_read(haptics, RAM_DATA_REG);
		} else if (len == 4) {
			drv2604l_reg_write(haptics, RAM_ADDR_UPPER_BYTE_REG, buff[2]);
			drv2604l_reg_write(haptics, RAM_ADDR_LOWER_BYTE_REG, buff[1]);
			haptics->r_len = (buff[3]>MAX_READ_BYTES)?MAX_READ_BYTES:buff[3];
			for(i=0; i < haptics->r_len; i++){
				haptics->r_buff[i] = drv2604l_reg_read(haptics, RAM_DATA_REG);
			}
		} else {
	...
	case HAPTIC_CMDID_REG_READ: {
		if (len == 2) {
			haptics->r_len = 1;
			haptics->r_buff[0] = drv2604l_reg_read(haptics, buff[1]);
		} else if (len == 3) {
			haptics->r_len = (buff[2]>MAX_READ_BYTES)?MAX_READ_BYTES:buff[2];
			drv2604l_bulk_read(haptics, buff[1], haptics->r_len, haptics->r_buff);
		} else {
	...
	case HAPTIC_CMDID_REG_WRITE: {
		if ((len-1) == 2) {
			drv2604l_reg_write(haptics, buff[1], buff[2]);
		} else if ((len-1)>2) {
			...
			if (data != NULL) {
				if (copy_from_user(data, &buff[2], len-2) != 0) {
					...
					drv2604l_bulk_write(haptics, buff[1], len-2, data);
				}
				kfree(data);
			}
		} else {
	...
	case HAPTIC_CMDID_REG_SETBIT: {
		int i=1;
		for (i=1; i< len; ) {
			drv2604l_set_bits(haptics, buff[i], buff[i+1], buff[i+2]);
			i += 3;
		}
		break;
	}
...
static struct file_operations fops =
{
	.open = dev2604l_open,
	.read = dev2604l_read,
	.write = dev2604l_write,
};

#ifdef CONFIG_HAS_EARLYSUSPEND
void drv2604l_early_suspend(struct early_suspend *h){
	struct drv2604l_data *haptics = container_of(h, struct drv2604l_data, early_suspend);
...
static int drv2604l_init(struct drv2604l_data *haptics)
{
	int reval = -ENOMEM;

	haptics->version = MKDEV(0,0);
	reval = alloc_chrdev_region(&haptics->version, 0, 1, "drv2604l");
	if (reval < 0) {
	...
	haptics->device = device_create(haptics->class, NULL, haptics->version, NULL, "drv2604l");
	if (!haptics->device) {
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	haptics->early_suspend.suspend = drv2604l_early_suspend;
	haptics->early_suspend.resume = drv2604l_late_resume;
...
static void dev_init_platform_data(struct drv2604l_data *haptics)
{
	struct drv2604l_platform_data *pdata = &haptics->platform_data;
	...
	if (haptics->otp == 0) {
		if (actuator.rated_voltage != 0) {
			drv2604l_reg_write(haptics, RATED_VOLTAGE_REG, actuator.rated_voltage);
		} else {
			printk("%s, ERROR Rated ZERO\n", __FUNCTION__);
		}

		if(actuator.overdrive_voltage != 0) {
			drv2604l_reg_write(haptics, OVERDRIVE_CLAMP_VOLTAGE_REG, actuator.overdrive_voltage);
		} else {
			printk("%s, ERROR OverDriveVol ZERO\n", __FUNCTION__);
		}

		drv2604l_set_bits(haptics,
	...
	if ((pdata->loop_mode == CLOSE_LOOP)
		...
		drv2604l_reg_write(haptics, REAL_TIME_PLAYBACK_REG, 0xff);
	} else {
		if(pdata->rtp_format == SIGNED) {
			ctrlx_temp |= RTP_FORMAT_SIGNED;
			drv2604l_reg_write(haptics, REAL_TIME_PLAYBACK_REG, 0x7f);
		} else {
			ctrlx_temp |= RTP_FORMAT_UNSIGNED;
			drv2604l_reg_write(haptics, REAL_TIME_PLAYBACK_REG, 0xff);
		}
	...
	if (actuator.device_type == LRA) {
		...
		drv2604l_reg_write(haptics, LRA_OPENLOOP_PERIOD_REG, reg_20);
	}
}

#if SKIP_AUTOCAL == 0
static int dev_auto_calibrate(struct drv2604l_data *haptics)
{
	int err = 0, status=0;

	drv2604l_change_mode(haptics, WORK_CALIBRATION, DEV_READY);
	drv2604l_set_go_bit(haptics, GO);

	/* Wait until the procedure is done */
	drv2604l_poll_go_bit(haptics);
	/* Read status */
	status = drv2604l_reg_read(haptics, STATUS_REG);

	printk("%s, calibration status =0x%x\n", __FUNCTION__, status);

	/* Read calibration results */
	drv2604l_reg_read(haptics, AUTO_CALI_RESULT_REG);
	drv2604l_reg_read(haptics, AUTO_CALI_BACK_EMF_RESULT_REG);
	drv2604l_reg_read(haptics, FEEDBACK_CONTROL_REG);
 ...
 **/
static int drv2604l_calculate_voltage(unsigned int voltage)
{
	return (voltage * 255 / 5600);
}

#ifdef CONFIG_OF
static int drv2604l_parse_dt(struct device *dev,
		struct drv2604l_data *haptics)
{
	...
	if (!error)
		pdata->actuator.rated_voltage = drv2604l_calculate_voltage(voltage);

	error = of_property_read_u32(np, "vib-overdrive-mv", &voltage);
	if (!error)
		pdata->actuator.overdrive_voltage= drv2604l_calculate_voltage(voltage);
...
#else
static inline int drv2604l_parse_dt(struct device *dev,
		struct drv2604l_data *haptics)
{
...
{
	struct drv2604l_data *haptics;
	struct drv2604l_platform_data *pdata = dev_get_platdata(&client->dev);
	unsigned int reg_val = 0;
	int err = 0;

	printk(KERN_DEBUG"%s:Enter\n", __FUNCTION__);

	haptics = devm_kzalloc(&client->dev, sizeof(struct drv2604l_data), GFP_KERNEL);
	if (haptics == NULL) {
	...
	if (pdata) {
		memcpy(&haptics->platform_data, pdata, sizeof(struct drv2604l_platform_data));
	}
	else if (client->dev.of_node) {
		err = drv2604l_parse_dt(&client->dev, haptics);
		if (err)
			return err;
	}
	...
	haptics->regmap = devm_regmap_init_i2c(client, &drv2604l_i2c_regmap);
	...
	reg_val = drv2604l_reg_read(haptics, STATUS_REG);
...
*/
	...
	haptics->otp = drv2604l_reg_read(haptics, CTRL4_REG) & CTRL4_REG_OTP_MASK;
	...
	drv2604l_init(haptics);

	drv2604l_data = haptics;
...
{
	struct drv2604l_data *haptics = i2c_get_clientdata(client);
...
static int __init drv2604x_init(void)
{
	return i2c_add_driver(&drv2604l_driver);
}

static void __exit drv2604x_exit(void)
{
	i2c_del_driver(&drv2604l_driver);
}

module_init(drv2604x_init);
module_exit(drv2604x_exit);
