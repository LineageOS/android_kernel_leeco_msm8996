...
static unsigned char CTPM_FW[] = {
	#include "FT_Upgrade_App.img"
};

static unsigned char aucFW_PRAM_BOOT[] = {
	#include "FT8xx6_Pramboot_Vx.x_xxxx.img"
};
...
int  fts_8716_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (j = 0; j < packet_number; j++) {
		...
		if( 30 == i)
			FTS_DBG("Query 6a reg time out value: 0x%x, driver set value:0x%x\n",  temp, uCheckOff + uCheckStart);
	}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if (uc_tp_fm_ver == FTS_REG_FW_VER ||	uc_tp_fm_ver < uc_host_fm_ver || uc_tp_fm_ver == 0 || uc_tp_fm_ver == 0xFF)
	{
		msleep(100);
		dev_dbg(&client->dev, "[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
		pr_err("[FTS] Start upgrade FW  \n ");
		do{
			i_ret = fts_ctpm_fw_upgrade_with_i_file(client);
		}while((i_ret != 0) && (retry-- >0));
		pr_err("[FTS] update fw retry = %d \n",3-retry);
		if (i_ret == 0)
		{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			dev_dbg(&client->dev, "[FTS] upgrade to new version 0x%x\n",uc_host_fm_ver);
			pr_err("[FTS] upgrade FW success ! \n ");
		}
		else
		{
			pr_err("[TP-0x1202] [FTS] upgrade failed ret=%d.\n", i_ret);
			return -EIO;
		}
	}
	return 0;
