...
/*****************************************************************************
 * /sys/android_touch/ attributes patch
*****************************************************************************/
static struct kset *g_android_touch;

static int g_i2c_flag = 0;
...
static void fts_touch_irq_work(void);
...
int fts_i2c_read(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen)
{
	...
	if(readlen > 0)
	{
		if (writelen > 0) {
			...
			if (ret < 0) {
				if (1 == g_i2c_flag) {
					dev_err(&client->dev, "[TP-0x1201] %s: i2c read error.\n", __func__);
				} else {
...
int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
	...
	if(writelen > 0)
	{
		...
		if (ret < 0) {
			if (1 == g_i2c_flag) {
				dev_err(&client->dev, "[TP-0x1201] %s: i2c write error.\n", __func__);
			} else {
...
static int fts_get_key_index(const struct fts_ts_platform_data *pdata, unsigned int x, unsigned int y)
{
	int i;
	...
	for (i=0; i<pdata->keymap_num; i++){
		if ((x > (pdata->keymap_range[i].x - pdata->keymap_range[i].x_width / 2))
				&& (x < (pdata->keymap_range[i].x + pdata->keymap_range[i].x_width / 2))
				&& (y > (pdata->keymap_range[i].y - pdata->keymap_range[i].y_width / 2))
				&& (y < (pdata->keymap_range[i].y + pdata->keymap_range[i].y_width / 2)) ) {
			return i;
		}
	}
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	for (i = 0; i < event->touch_point; i++)
	{
		...
		if (event->au8_touch_event[i] == FTS_TOUCH_DOWN || event->au8_touch_event[i] == FTS_TOUCH_CONTACT) {
			index = fts_get_key_index(pdata, (unsigned int)event->au16_x[i],
												(unsigned int)event->au16_y[i]);
			if (index >= 0) {
				input_event(data->input_dev, EV_KEY, pdata->keymap_value[index], 1);
				touchs |= BIT(event->au8_finger_id[i]);
				data->touchs |= BIT(event->au8_finger_id[i]);
				key_index |= BIT(index);
				data->key_index |= BIT(index);
				continue;
			}
		} else {
			index = fts_get_key_index(pdata, (unsigned int)event->au16_x[i],
			...
			if (index >= 0) {
				uppoint++;
				input_event(data->input_dev, EV_KEY, pdata->keymap_value[index], 0);
				data->key_index &= ~BIT(index);
				data->touchs &= ~BIT(event->au8_finger_id[i]);
				continue;
			}
	...
	/* add key function */
	if(unlikely(data->key_index ^ key_index))
	{
		for(i = 0; i < pdata->keymap_num; i++)
		{
			if(BIT(i) & (data->key_index ^ key_index))
			{
				input_event(data->input_dev, EV_KEY, pdata->keymap_value[i], 0);
			}
		}
	}
...
static int fts_power_on(struct fts_ts_data *data, bool on)
{
	...
	if ((data->vdd != NULL) && (regulator_count_voltages(data->vdd) > 0)) {
...
static int fts_power_init(struct fts_ts_data *data, bool on)
{
	...
	if (of_find_property(data->client->dev.of_node, "vdd-supply", NULL)) {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	/* add key function */
	rc = of_property_read_u32(np, "focaltech,keymap-num", &pdata->keymap_num);
	if (rc) {
		pdata->keymap_num = 0;
	}
	if (pdata->keymap_num > MAX_BUTTONS) {
		pdata->keymap_num = 0;
	}
	if (0 == pdata->keymap_num) {
		return 0;
	}
	pdata->keymap_value = devm_kzalloc(dev,
			(sizeof(pdata->keymap_value) * pdata->keymap_num), GFP_KERNEL);
	if (!pdata->keymap_value) {
		pdata->keymap_value = 0;
		dev_err(dev, "devm_kzalloc keymap_value failed\n");
		return 0;
	}

	pdata->keymap_range = devm_kzalloc(dev,
			(sizeof(pdata->keymap_range) * pdata->keymap_num), GFP_KERNEL);
	if (!pdata->keymap_range) {
		pdata->keymap_value = 0;
		dev_err(dev, "devm_kzalloc keymap_range failed\n");
		return 0;
	}

	rc = of_property_read_u32_array(np, "focaltech,keymap-value",
									button_map, pdata->keymap_num);
	if (rc) {
		dev_err(dev, "Unable to read key code\n");
		pdata->keymap_num = 0;
		return 0;
	}
	for (i=0; i<pdata->keymap_num; i++) {
		pdata->keymap_value[i] = button_map[i];
	}

	rc = of_property_read_u32_array(np, "focaltech,keymap-range",
									button_map, (pdata->keymap_num * 4));
	if (rc) {
		dev_err(dev, "Unable to read key range\n");
		pdata->keymap_num = 0;
		return 0;
	}
	for (i=0; i<pdata->keymap_num; i++) {
		pdata->keymap_range[i].x = button_map[4*i+0];
		pdata->keymap_range[i].y = button_map[4*i+1];
		pdata->keymap_range[i].x_width = button_map[4*i+2];
		pdata->keymap_range[i].y_width = button_map[4*i+3];
	}
...
static ssize_t android_touch_vendor_show(struct device *dev, 
			struct device_attribute *attr, char *buf)
{
	...
	retval = snprintf(buf, PAGE_SIZE, "focaltech_FT%02x_0x%02x\n", chip_id, fwver);
	return retval;
}

static struct device_attribute g_touch_attrs[] = {
	__ATTR(vendor, (S_IRUGO), 
			android_touch_vendor_show,
			NULL),
};

static void fts_update_fw_work(struct work_struct *work)
{
	fts_ctpm_auto_upgrade(fts_wq_data->client);
...
static void fts_suspend_work(struct work_struct *work)
{
	...
	data = container_of(work, struct fts_ts_data, suspend_work);
	fts_ts_suspend(&data->client->dev);
}
...
{
	struct fts_ts_data *data;
	data = container_of(work, struct fts_ts_data, resume_work);
	fts_ts_resume(&data->client->dev);
}
/*******************************************************************************
*  Name: fts_ts_probe
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	for (i = 0; i < pdata->keymap_num; i++) {
		set_bit(pdata->keymap_value[i], input_dev->keybit);
		input_set_capability(input_dev, EV_KEY, pdata->keymap_value[i]);
	}
	...
	g_android_touch = kset_create_and_add("android_touch", NULL, NULL);

	for(attr_count=0; attr_count < ARRAY_SIZE(g_touch_attrs); attr_count++) {
		err = sysfs_create_file(&g_android_touch->kobj, 
							&g_touch_attrs[attr_count].attr);
		if (err < 0) {
	...
	INIT_WORK(&data->suspend_work,fts_suspend_work);
	INIT_WORK(&data->resume_work, fts_resume_work);
	return 0;

err_sysfs:
	for (attr_count--; attr_count >= 0; attr_count--) {
		sysfs_remove_file(&g_android_touch->kobj,
				&g_touch_attrs[attr_count].attr);
	}
