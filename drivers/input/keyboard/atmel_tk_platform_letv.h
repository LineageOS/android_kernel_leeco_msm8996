...
static struct ts_data *ts_g_data;
#endif
...
static inline void board_gpio_init(const struct ts_platform_data *pdata)
{
	...
	//don't use the read_chg in ts_wait_for_chg, because the msg can be readout by interrupt thread
...
static inline void board_gpio_deinit(const struct ts_platform_data *pdata)
{
#if defined(CONFIG_MXT_PLATFORM_QUALCOMM)
...
static void board_init_irq(const struct ts_platform_data *pdata)
{
...
static inline void board_enable_irq(const struct ts_platform_data *pdata, unsigned int irq)
{
#if defined(CONFIG_MXT_PLATFORM_MTK)
...
static inline void board_enable_irq_wake(const struct ts_platform_data *pdata, unsigned int irq)
{
#if defined(CONFIG_MXT_PLATFORM_MTK)
#else
	enable_irq_wake(irq);
...
static inline void board_disable_irq_wake(const struct ts_platform_data *pdata, unsigned int irq)
{
#if defined(CONFIG_MXT_PLATFORM_MTK)
#else
...
static inline void board_disable_irq(const struct ts_platform_data *pdata, unsigned int irq)
{
#if defined(CONFIG_MXT_PLATFORM_MTK)
	mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
#else
...
static inline void board_free_irq(const struct ts_platform_data *pdata, unsigned int irq, void *dev_id)
{
#if defined(CONFIG_MXT_PLATFORM_MTK)
	mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
#else
