...
struct dentry *rear_cam_name = NULL;
struct dentry *front_cam_name = NULL;
struct msm_eeprom_ctrl_t *rear_cam_ctrl = NULL;
struct msm_eeprom_ctrl_t *front_cam_ctrl = NULL;

static ssize_t rear_cam_read(struct file *fp, char __user *user_buffer,
		size_t count, loff_t *position)
{
	int j;

	for (j = 0; j < rear_cam_ctrl->cal_data.num_data; j++)
		pr_info("memory_data[%d] = 0x%X\n", j,
				rear_cam_ctrl->cal_data.mapdata[j]);
	return 0;
}

static ssize_t front_cam_read(struct file *fp, char __user *user_buffer,
		size_t count, loff_t *position)
{
	int j;

	for (j = 0; j < front_cam_ctrl->cal_data.num_data; j++)
		pr_info("memory_data[%d] = 0x%X\n", j,
				front_cam_ctrl->cal_data.mapdata[j]);
	return 0;
}

static const struct file_operations rear_cam_operations = {
	.read = rear_cam_read,
};

static const struct file_operations front_cam_operations = {
	.read = front_cam_read,
};
...
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	if (rear_cam_name == NULL) {
		rear_cam_name = debugfs_create_dir(eb_info->eeprom_name, dirret);
		rear_cam_ctrl = e_ctrl;
		debugfs_create_file("memory_data", S_IFREG | S_IRUGO, rear_cam_name, NULL,
				&rear_cam_operations);
	} else {
		front_cam_name = debugfs_create_dir(eb_info->eeprom_name, dirret);
		front_cam_ctrl = e_ctrl;
		debugfs_create_file("memory_data", S_IFREG | S_IRUGO, front_cam_name, NULL,
				&front_cam_operations);
	}
	CDBG("%s X\n", __func__);
	return rc;
