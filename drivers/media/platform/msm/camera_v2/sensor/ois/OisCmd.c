...
// Function Name 	: Smooth Servo Off
...
// History			: First edition 						2016.01.21 Rex
...
void SmoothSvrOff(void)
{
	...
	for (UsStep = 0; UcCnt < 200; UcCnt++) {
		...
		RamWriteA( SYLMT, UsDrv ) ;			// 0x14F7
		WitTim(1);
	}
	RamWriteA( SXLMT, 0x00 ) ;				// 0x1477
	RamWriteA( SYLMT, 0x00 ) ;				// 0x14F7
	RamAccFixMod( OFF ) ;					// Float mode	
}
//#endif	//AF_MID_MOUNT
