...
void sp_tx_get_rx_bw(unsigned char *bw)
{
    ...
    */
    ...
    if (c <= c1)
              c = c1;
    *bw = c;
    pr_info("%s %s : sp_tx_get_rx_bw %x\n", LOG_TAG, __func__,(unsigned int) *bw);
...
unchar *hdmi_get_edid_buf(void)
{
    return edid_blocks; 
}
EXPORT_SYMBOL(hdmi_get_edid_buf);
