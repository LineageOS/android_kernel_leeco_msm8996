...
static struct lm3697_bl *lm3697_backlight ;
...
int lm3697_bl_set(int bl_level)
{
	int ret = 0;

	if (bl_level > 0)
		ret = lm3697_bl_enable(lm3697_backlight, 1);
	else
		ret = lm3697_bl_enable(lm3697_backlight, 0);
			   ...
			   const struct i2c_device_id *id)
{
	...
	lm3697_bl_write_byte(chip, 0x1a, 0x06);
	lm3697_bl_write_byte(chip, 0x16, 0x01);
