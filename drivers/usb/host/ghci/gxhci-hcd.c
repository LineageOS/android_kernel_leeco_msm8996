...
#define DRIVER_DESC    "Generic Host Controller for USB3.0"
#define DRIVER_VERSION "1.2.0a03"

static const char driver_name[] = "gxhci_hcd";
static const char driver_desc[] = DRIVER_DESC;
static const char device_name[] = "gxhci_device";
...
static const char *ghci_get_speed_string(int speed)
{
	...
	switch (speed) {
	case GHCI_SPEED_SUPER:str = "super";   break;
...
static int ghci_connect_device(struct ghci_port *port, struct ghci_device *dev)
{
	...
	switch (port->speed) {
	case GHCI_SPEED_SUPER:
		port->status |= USB_PORT_STAT_SPEED_5GBPS;
		break;
...
static void ghci_ss_hub_descriptor(
	struct usb_hcd *hcd,
	struct usb_hub_descriptor *desc)
{
	struct ghci_platform_hcd *priv = hcd->self.controller->platform_data;
	...
	desc->bPwrOn2PwrGood = 0;
	desc->bHubContrCurrent = 0;
	desc->u.ss.bHubHdrDecLat = 0x04; /* Worst case 0.4u sec */
	...
	u16 wLength)
{
	...
	case ClearPortFeature:
		...
		{
			dev_err(hcd->self.controller,
				"USB_PORT_FEAT_SUSPEND not supported for USB3.0 root hub\n");
			goto error;
		}
	...
	case GetHubDescriptor:
		...
		{
			dev_err(hcd->self.controller,
				"Invalid hub descriptor type for USB 3.0 root hub\n");
			goto error;
		}

		if (hcd->speed == HCD_USB3) {
			ghci_ss_hub_descriptor(hcd, (struct usb_hub_descriptor *)buf);
		}
		else {
	...
	case DeviceRequest | USB_REQ_GET_DESCRIPTOR:
		i = (wIndex & 0x00ff) - 1; /* port index */
		dev_dbg(hcd->self.controller,
			"DeviceRequest | USB_REQ_GET_DESCRIPTOR (BOS descriptor)\n");
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_LINK_STATE:
			if (hcd->speed != HCD_USB3) {
				dev_err(hcd->self.controller,
					"USB_PORT_FEAT_LINK_STATE not supported "
					"for USB 2.0 root hub\n");
				goto error;
			}
			break;
		...
		case USB_PORT_FEAT_U2_TIMEOUT:
			if (hcd->speed != HCD_USB3) {
				dev_err(hcd->self.controller,
					"USB_PORT_FEAT_U1/2_TIMEOUT not supported "
					"for USB 2.0 root hub\n");
				goto error;
			}
			break;
		case USB_PORT_FEAT_SUSPEND:
			if (hcd->speed == HCD_USB3) {
				dev_err(hcd->self.controller,
					"USB_PORT_FEAT_SUSPEND not supported "
					"for USB 3.0 root hub\n");
				goto error;
			}
			break;
		case USB_PORT_FEAT_POWER:
			spin_lock_irqsave(&ghci->lock, flags);
			if (hcd->speed == HCD_USB3) {
				ghci->port[i].status |= USB_SS_PORT_STAT_POWER;
			}
			else {
				ghci->port[i].status |= USB_PORT_STAT_POWER;
			}
			spin_unlock_irqrestore(&ghci->lock, flags);
			break;
		case USB_PORT_FEAT_BH_PORT_RESET:
			if (hcd->speed != HCD_USB3) {
				dev_err(hcd->self.controller,
					"USB_PORT_FEAT_BH_PORT_RESET not supported "
					"for USB 2.0 root hub\n");
				goto error;
			}
			/* FALLS THROUGH */
	...
	case GetPortErrorCount:
		...
		dev_dbg(hcd->self.controller,
			"GetPortErrorCount port %d value 0x%04x\n",
			i + 1, wValue);

		if (hcd->speed != HCD_USB3) {
			dev_err(hcd->self.controller,
				"GetPortErrorCount not supported "
				"for USB 2.0 root hub\n");
			goto error;
		}
	...
	case SetHubDepth:
		...
		dev_dbg(hcd->self.controller,
			"SetHubDepth port %d value 0x%04x\n",
			i + 1, wValue);

		if (hcd->speed != HCD_USB3) {
			dev_err(hcd->self.controller,
				"SetHubDepth not supported "
				"USB 2.0 root hub\n");
			goto error;
		}
		break;
...
static int ghci_ioctl_claim_port(struct file *file, unsigned int speed)
{
	...
	if ((speed < GHCI_SPEED_LOW) || (speed > GHCI_SPEED_SUPER)) {
		return -EINVAL;
	}
...
static int __init ghci_device_register(dev_t devt)
{
	...
	ctl_dev = device_create(cls, NULL, devt, "gxhci_ctl");
	if (IS_ERR(ctl_dev)) {
