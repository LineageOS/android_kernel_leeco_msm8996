...
unsigned char letv_pd_aes_key[] = {
0x66, 0x78, 0x61, 0x6A, 0x6C, 0x77, 0x77, 0x62,
0x66, 0x6B, 0x6A, 0x6A, 0x63, 0x66, 0x78, 0x73};
...
/* expand the key */
void letv_pd_expandKey(unsigned char *expandedKey, unsigned char *key)
{
...
*/
void letv_pd_aes_encr(unsigned char *state, unsigned char *expandedKey)
{
...
no structuring with "for (....)" to save cycles
*/
void letv_pd_aes_decr(unsigned char *state, unsigned char *expandedKey)
{
...
void letv_pd_aes_encrypt(unsigned char *data, int size)
{
	unsigned char expandedKey[176];
	if (size != LETV_VDM_ENCRPT_DATA_LEN) {
		pr_err("%s:ERROR: size fail\n", __func__);
		return;
	}

	letv_pd_expandKey(expandedKey, letv_pd_aes_key);       /* expand the key into 176 bytes */
	letv_pd_aes_encr(data, expandedKey);
	return;
}

void letv_pd_aes_decrypt(unsigned char *data)
{
	unsigned char expandedKey[176];

	letv_pd_expandKey(expandedKey, letv_pd_aes_key);       /* expand the key into 176 bytes */
	letv_pd_aes_decr(data, expandedKey);
	return;
}
