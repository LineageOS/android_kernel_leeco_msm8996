...
static int __init get_bootparam_console_state(char *str)
{
        strlcpy(g_console_state, str, CONSOLE_STATE_CMDLINE_MAX);
        return 1;
}
__setup("android.letv.serial_console=", get_bootparam_console_state);
#endif
	...
	if (g_console_name != NULL && g_console_state != NULL) {
		if (!strncmp(g_console_state, "enabled", 7)) {
			...
			if (!strncmp(g_console_name, dev_name, 7)) {
				...
				if (pdev->dev.of_node) {
					console_uart_pins = devm_pinctrl_get(&pdev->dev);
					if (IS_ERR_OR_NULL(console_uart_pins)) {
						pr_err("cannot get ttyHSL%d pinctrl.\n", line);
					} else {
						set_state = pinctrl_lookup_state(console_uart_pins, "sleep");
						if (IS_ERR(set_state))
