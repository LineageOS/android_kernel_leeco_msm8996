...
*/
#define P61_DBG_MSG P61_ERR_MSG
        ...
        unsigned long arg)
{
   ...
   // P61_DBG_MSG(KERN_ALERT "p61_dev_ioctl-Enter %u arg = %ld\n", cmd, arg);
    ...
    switch (cmd) {
    case P61_SET_PWR:
        P61_ERR_MSG("%s: P61_SET_PWR = %ld\n", __func__, P61_SET_PWR);
        if (arg == 2)
        {
#ifdef P61_HARD_RESET
    ...
    case P61_SET_DBG:
        P61_ERR_MSG("%s: P61_SET_DBG = %ld\n", __func__, P61_SET_DBG);
    ...
    case P61_SET_POLL:
        ...
        P61_ERR_MSG("%s: P61_SET_POLL= %ld\n", __func__, P61_SET_POLL);
        ...
        loff_t *offset)
{
    ...
    P61_DBG_MSG(KERN_ALERT "p61_dev_write: count=%zu\n", count);
    ...
    print_packet(tx_buffer,count,1);
    mutex_unlock(&p61_dev->write_mutex);
    P61_DBG_MSG(KERN_ALERT "p61_dev_write: exit. ret=%d\n", ret);
    return ret;
}
        ...
        loff_t *offset)
{
    ...
    P61_DBG_MSG("p61_dev_read: enter. count=%zu\n", count);
    ...
    else
    {
        ...
        {
	    P61_ERR_MSG("%s: Interrupt Mode. irq_gpio = 0\n", __func__);
            if (filp->f_flags & O_NONBLOCK)
            {
				P61_DBG_MSG(" %s Interrupt Mode NONBLOCK\n", __FUNCTION__);
                ret = -EAGAIN;
                goto fail;
            }
            while (1)
        ...
        do {
            ...
            if ((0 > ret) || (sof != SOF))
            {
                P61_ERR_MSG(KERN_ALERT "%s SOF or ret error. [sof] = 0x%x ret = %d\n", __FUNCTION__, sof, ret);
                ret = -EIO;
                goto fail;
            }
            ++i;
    ...
    /* Get the data length */
    count = rx_buffer[2];
    /* P61_DBG_MSG(KERN_INFO "Data Lenth = %zu\n", count); */
    ...
    /* P61_DBG_MSG(KERN_INFO "total_count = %d\n", total_count); */

    print_packet(rx_buffer,total_count,0);

    ret = copy_to_user(buf, &rx_buffer[0], total_count);

    if (ret)
    {
        P61_ERR_MSG("%s : failed to copy to user space,ret %d\n", __func__,ret);
        ret = -EFAULT;
        goto fail;
    }
    ret = total_count;
    ...
    fail:
		P61_ERR_MSG("p61_dev_read: Read Error. ret=%d\n", ret);
		mutex_unlock(&p61_dev->read_mutex);
