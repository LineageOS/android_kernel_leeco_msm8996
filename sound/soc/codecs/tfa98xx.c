				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, start_cmd, sizeof(start_cmd) - 1)) {
		...
		queue_delayed_work(tfa98xx->tfa98xx_wq,
					&tfa98xx->monitor_work, msecs_to_jiffies(1000));
...
static void tfa98xx_tapdet_check_update(struct tfa98xx *tfa98xx)
{
	...
	if (!gpio_is_valid(tfa98xx->irq_gpio)) {
		...
		if (enable)
			queue_delayed_work(tfa98xx->tfa98xx_wq,
						&tfa98xx->tapdet_work, msecs_to_jiffies(100));
		else
...
static void tfa98xx_tapdet_work(struct work_struct *work)
{
	...
	queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->tapdet_work, msecs_to_jiffies(100));
}
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	/* reschedule */
	queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->monitor_work, msecs_to_jiffies(5000));
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (no_start == 0) {
		...
		case 0x88: /* tfa9888 */
			...
			if ((reg & 0xfff) == 0xc88) {
				fw_name = "tfa98xx_N1C.cnt";
				pr_info("TFA9888 detected%s\n",fw_name);
			} else if ((reg & 0xfff) == 0xb88){
				fw_name = "tfa98xx_N1B.cnt";
				pr_info("TFA9888 detected%s\n",fw_name);
			}
			break;
		case 0x80: /* tfa9890 */
